...
bool tools::isThinLTOSplitMergeEnabled(const ToolChain &TC,
  ...
  // output; without -c the object feeds a link step and there is no merge
  // action (link-time splitting is then handled inside the linker itself). This
  // is the single gating predicate shared by the cc1 flag and the merge action.
