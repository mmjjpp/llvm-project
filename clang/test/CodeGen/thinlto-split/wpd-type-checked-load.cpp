...
// TypeCheckedLoadVCalls (variable args) and TypeCheckedLoadConstVCalls
// (constant args) from the function summary's TypeIdInfo.
//
// -fsanitize=cfi-vcall (with -fwhole-program-vtables) causes Clang to emit
// llvm.type.checked.load. Variable args -> TypeCheckedLoadVCalls,
// constant args -> TypeCheckedLoadConstVCalls.
//
...
// the caller passes, so it must emit type.checked.load for CFI/WPD.

// RUN: rm -rf %t %t.dir && split-file %s %t.dir
// RUN: %clangxx -O2 -flto=thin -fwhole-program-vtables -fvisibility=hidden \
// RUN:   -fsanitize=cfi-vcall -fsanitize-trap=cfi-vcall \
...
// Verify summary has TypeCheckedLoadVCalls and TypeCheckedLoadConstVCalls
// RUN: llvm-dis %t.index.bc -o - 2>/dev/null | FileCheck %s --check-prefix=SUMMARY
// SUMMARY-DAG: typeCheckedLoadVCalls
// SUMMARY-DAG: typeCheckedLoadConstVCalls

// Verify split produced partition files
// RUN: ls %t.lto.*.o
...
// Variable arg -> TypeCheckedLoadVCalls
int caller_var(const Base *p, int x) { return p->fn(x); }

// Constant arg -> TypeCheckedLoadConstVCalls
