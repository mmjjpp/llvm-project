...
static cl::opt<int> LTOSplitPartitions(
    "lto-split-partitions", cl::Hidden, cl::init(1),
    cl::desc("Number of partitions to split each module into in the LTO "
             "backend (regular and ThinLTO) using call-graph-based "
             "splitting. 1 (the default) disables splitting. Values > 1 "
             "enable splitting into up to that many partitions (capped by "
             "the number of call-graph roots). Values <= 0 are illegal."));

// Derived from LTOSplitPartitions: true iff call-graph-based splitting is
// enabled (LTOSplitPartitions > 1). Set at the start of lto::backend /
// lto::thinBackend after command-line parsing, because cl::opt values are
// only available at runtime and cannot be used to initialize a global at
// static-initialization time. Reading the semantic bool here keeps the rest
// of the code readable instead of repeating the numeric threshold.
static bool LTOSplitByCG = false;
...
Error lto::backend(const Config &C, AddStreamFn AddStream,
  ...
  if (LTOSplitPartitions < 1)
    report_fatal_error("-lto-split-partitions must be >= 1 (1 disables "
                       "splitting, >1 enables it)");
  LTOSplitByCG = LTOSplitPartitions > 1;
